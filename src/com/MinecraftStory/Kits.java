...
public class Kits extends JavaPlugin {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("kit") && sender instanceof Player) {
			try {
				if (args.length == 3) {
					if(sender.hasPermission("minecraftstory.kits.admin") && args[0].matches("^[A-Za-z0-9]{1,10}$") && args[1].matches("^[A-Za-z0-9]{1,10}$") && args[2].matches("^[0-9]{1,32}$")) {
				...
				if (args.length == 2) {
					if(sender.hasPermission("minecraftstory.kits.admin") && args[0].matches("^[A-Za-z0-9]{1,10}$") && args[1].matches("^[A-Za-z0-9_]{1,10}$")) {
						if(args[0].equalsIgnoreCase("set") && !args[1].equalsIgnoreCase("time")) {
				...
				if (args.length == 1) {
					if(sender.hasPermission("minecraftstory.kits.kit."+args[0]) && args[0].matches("^[A-Za-z0-9]{1,10}$")) {
						InventoryNBTSer.getKit((Player) sender, folder, args[0], sender.hasPermission("minecraftstory.kits.bypass"));
						return true;
					} else {
				...
				if (args.length == 0) {
					...
					for(String s : InventoryNBTSer.getLogs(folder)) {
						if(sender.hasPermission("minecraftstory.kits.kit."+s)) {
							l.add(s);
						}
					}
